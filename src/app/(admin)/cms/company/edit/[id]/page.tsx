...
function EditCompanyInfo({ params }: { params: { id: string } }) {
  const utils = trpc.useUtils()
  const { data, isLoading } = trpc.cms.company.getCompanyId.useQuery({
    id: params.id
  })
  const { mutate, isPending } = trpc.cms.company.updateCompany.useMutation({
    onSuccess: () => {
      utils.cms.company.getCompanyId.invalidate({
        id: params.id
      })
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className='col-span-1 flex flex-col space-y-3 mb-3'>
          ...
          <Input onChange={e => debounceEdit({
            name: e.target.value
          })} defaultValue={data.name} type='text' name="Company Name" required />
        </div>
        <div className='col-span-1 flex flex-col space-y-3 mb-3'>
          <Label>
            Company Url
          ...
          <Input onChange={e => debounceEdit({
            link: e.target.value
          })} defaultValue={data.link || ""} type='url' name="Company Name" required />
        </div>
      </div>
